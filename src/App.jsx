...
function ReportsView({txns,catMap,curMonth,setCurMonth,accMap,accounts,reportTab,setReportTab}) {
  ...
  const nwMonths=useMemo(()=>months6.map(mk=>{
    ...
    accounts.forEach(a=>{
      let bal=0;
      txns.filter(t=>t.accountId===a.id&&t.date<=mk+"-31").forEach(t=>{bal+=(t.type==="inflow"?t.amount:-t.amount);});
      // Credit cards, mortgages, loans typically carry negative balances (debts)
      // Assets: checking, savings, cash, asset, investment, other with positive balance
      const isDebtType = a.type==="credit"||a.type==="mortgage"||a.type==="loan";
      if(isDebtType){ debts+=Math.abs(Math.min(0,bal)); assets+=Math.max(0,bal); }
      else { assets+=Math.max(0,bal); debts+=Math.abs(Math.min(0,bal)); }
    });
    return{mk,assets,debts,total:assets-debts};
  ...
  return(<div>
    ...
    {reportTab==="net_worth"&&(()=>{
      const latest=nwMonths[nwMonths.length-1]||{assets:0,debts:0,total:0};
      const barMax=Math.max(...nwMonths.map(x=>Math.max(x.assets,x.debts)),1);
      return <div>
      <div style={{display:"flex",justifyContent:"center",gap:32,marginBottom:24,flexWrap:"wrap"}}>
        <div style={{textAlign:"center"}}><div style={{fontSize:11,color:"#64748b",marginBottom:2}}>Assets</div><div style={{fontSize:24,fontWeight:700,fontFamily:"'JetBrains Mono',monospace",color:"#22c55e"}}>{fmt(latest.assets)}</div></div>
        <div style={{textAlign:"center"}}><div style={{fontSize:11,color:"#64748b",marginBottom:2}}>Liabilities</div><div style={{fontSize:24,fontWeight:700,fontFamily:"'JetBrains Mono',monospace",color:"#ef4444"}}>{fmt(latest.debts)}</div></div>
        <div style={{textAlign:"center"}}><div style={{fontSize:11,color:"#64748b",marginBottom:2}}>Net Worth</div><div style={{fontSize:24,fontWeight:700,fontFamily:"'JetBrains Mono',monospace",color:latest.total>=0?"#a5b4fc":"#ef4444"}}>{fmt(latest.total)}</div></div>
      </div>
      <div style={{display:"flex",gap:12,justifyContent:"center",alignItems:"flex-end",height:220,marginBottom:8,padding:"0 8px"}}>
        {nwMonths.map(x=><div key={x.mk} style={{display:"flex",flexDirection:"column",alignItems:"center",flex:1,maxWidth:100,gap:0}}>
          {/* Asset bar (grows up) */}
          <div style={{width:"100%",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"flex-end",height:90}}>
            <span style={{fontSize:9,color:"#22c55e",fontFamily:"'JetBrains Mono',monospace",marginBottom:2}}>{x.assets>0?fmt(x.assets):""}</span>
            <div style={{width:"60%",background:"#22c55e25",borderRadius:"4px 4px 0 0",height:`${Math.max(2,(x.assets/barMax)*75)}px`,border:"1px solid #22c55e40",borderBottom:"none"}}/>
          </div>
          {/* Center line + label */}
          <div style={{width:"100%",borderTop:"2px solid #334155",padding:"2px 0",textAlign:"center"}}>
            <span style={{fontSize:10,color:"#94a3b8",fontWeight:500}}>{monthLabel(x.mk).split(" ")[0]}</span>
          </div>
          {/* Debt bar (grows down) */}
          <div style={{width:"100%",display:"flex",flexDirection:"column",alignItems:"center",height:90}}>
            <div style={{width:"60%",background:"#ef444425",borderRadius:"0 0 4px 4px",height:`${Math.max(2,(x.debts/barMax)*75)}px`,border:"1px solid #ef444440",borderTop:"none"}}/>
            <span style={{fontSize:9,color:"#ef4444",fontFamily:"'JetBrains Mono',monospace",marginTop:2}}>{x.debts>0?fmt(x.debts):""}</span>
          </div>
        </div>)}
      </div>
      {/* Legend */}
      <div style={{display:"flex",justifyContent:"center",gap:20,fontSize:12,color:"#94a3b8",marginBottom:20}}>
        <span><span style={{display:"inline-block",width:10,height:10,borderRadius:2,background:"#22c55e40",marginRight:4}}/>Assets</span>
        <span><span style={{display:"inline-block",width:10,height:10,borderRadius:2,background:"#ef444440",marginRight:4}}/>Liabilities</span>
      </div>
      {/* Table */}
      <div style={{maxWidth:500,margin:"0 auto"}}>
        <div style={{display:"flex",gap:8,padding:"8px 0",fontSize:10,fontWeight:600,textTransform:"uppercase",letterSpacing:".06em",color:"#64748b",borderBottom:"1px solid #334155"}}>
          <span style={{width:70}}>Month</span><span style={{flex:1,textAlign:"right"}}>Assets</span><span style={{flex:1,textAlign:"right"}}>Liabilities</span><span style={{flex:1,textAlign:"right"}}>Net Worth</span>
        </div>
        {nwMonths.map(x=><div key={x.mk} style={{display:"flex",gap:8,padding:"8px 0",fontSize:13,borderBottom:"1px solid #1e293b50"}}>
          <span style={{width:70,color:"#94a3b8"}}>{monthLabel(x.mk)}</span>
          <span style={{flex:1,textAlign:"right",color:"#22c55e",fontFamily:"'JetBrains Mono',monospace"}}>{fmt(x.assets)}</span>
          <span style={{flex:1,textAlign:"right",color:"#ef4444",fontFamily:"'JetBrains Mono',monospace"}}>{fmt(x.debts)}</span>
          <span style={{flex:1,textAlign:"right",color:x.total>=0?"#a5b4fc":"#ef4444",fontFamily:"'JetBrains Mono',monospace",fontWeight:600}}>{fmt(x.total)}</span>
        </div>)}
      </div>
    </div>;})()}
  </div>);
...
const S={
  app:{display:"flex",height:"100vh",fontFamily:"'DM Sans',sans-serif",background:"#0f172a",color:"#e2e8f0",overflow:"hidden"},
